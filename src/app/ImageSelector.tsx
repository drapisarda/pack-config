...
export default function ImageSelector(props: Props) {
  ...
  return (
    <div className="image-selector flex flex-wrap md:block h-full border rounded-xl border-slate-200 p-3 md:p-5 text-black bg-white md:mb-4 md:h-auto">
      ...
      <div className="image-selector__image my-2 md:my-4 basis-1/2">
        <Image 
          priority={true}
      ...
      <div className="image-selector__input order-4 w-full mt-1 md:w-auto text-center mb:py-2 md:mb-5">
        <label className="rounded-xl inline-block py-1 px-4 border border-slate-200 shadow-xl cursor-pointer" htmlFor={inputId}> Chose a new image </label>
        <input id={inputId} type="file" onChange={(e) => onChangeHandle(e)} className="hidden"/>
      </div>
      <p className="image-selector__description mt-2 md:mt-5 basis-1/2 order-3 px-2 md:px-0">
        {description}
      </p>
    </div>
  );
